...
- Start sequence: when beginning work on a picked task, announce the task you picked up, append a `started` event for the task (owner/agent id | <task name> | started, short reason), atomically move the task file from `./tmp/task` to `./tmp/task - active` (create the directory if missing) and re-read `task-monitor.md` tail to verify the `active` entry and the file presence in `./tmp/task - active`. If the file is already in `./tmp/task - active`, do not re-move or duplicate the `active` event, and then append `active` when processing begins. After appending `active`.
- Follow the prompt in the file to complete the task DO NOT STOP OR PAUSE. If the prompt requires user input, append `paused` and move the file to `./tmp/task - paused` until resumed.
- If file is missing during pickup or move, start over and choose a new file; do not append status events for missing files.
